...
# -*- coding: utf-8 -*-
"""
Created on Tue May 28 13:31:22 2024
...
def record_audio(filename_format, trial_info):
    duration= 0.5
    fs = 44100
    ...
    sf.write(filename_format.format(trial_info), recording, fs)
...
for trial_index in range(len(shuffled_sentences)):
    ...
    audio_filename_format = "audio_trial_{}.wav"
    audio_thread = threading.Thread(target=record_audio, args=(audio_filename_format, trial_index,))
    audio_thread.start()
    ...
    #wiat for audio recording to finish
    audio_thread.join()
